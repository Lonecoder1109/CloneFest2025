...
const TimelineItem: React.FC<TimelineItemProps> = ({ date, title, description, isLast = false }) => (
    <div className="relative flex items-start">
        <div className="flex flex-col items-center mr-6">
            <div className="flex-shrink-0 w-8 h-8 rounded-full bg-cyan-500 flex items-center justify-center text-slate-900 font-bold">
                {date.split(' ')[0][0]}
            </div>
            {!isLast && <div className="w-px h-full bg-slate-700 mt-2"></div>}
        </div>
        <div className="pb-16">
